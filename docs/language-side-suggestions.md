...
- 这已经足够支撑第一版 TUI。
- TUI 开启鼠标模式后，如果脚本异常退出但没有执行清理，终端会残留 mouse tracking，退出后继续输入会出现类似 `\x1b[<...M` 的鼠标事件文本。
...
- `session.onFocus()` 可后置
- `terminal.start({ mouse: true, alternateScreen: true, hideCursor: true })`：由标准库统一进入/退出这些终端模式。
- `session.restore()`：一次性恢复 raw mode、bracketed paste、mouse、cursor、alternate screen，并且可重复调用。

价值：

- 现在这些能力由 `src/tui/ansi.gs` 手写 escape 序列。
- 标准库封装后跨平台行为更可控，Windows Terminal、传统控制台、CI 非 TTY 可以统一处理。
- 退出路径由标准库兜底后，脚本层异常、回调异常、Ctrl+C 都更不容易把终端留在不可输入状态。

当前处理：

- `src/tui/runtime.gs` 已在脚本层用 `try/finally` 做恢复。
- 输入、resize、render、tick 回调增加异常边界，异常时先关闭鼠标和备用屏，再停止 session。
- 这仍属于应用层兜底；更可靠的长期方案应放到语言/标准库层。

建议测试：

- `terminal.start({ raw: true, bracketedPaste: true, mouse: true, alternateScreen: true })` 后抛错，进程退出前必须恢复所有模式。
- `onInput` 回调内部抛错时，标准库不应只打印错误，应能停止 session 或触发可配置的 `onError`。
- `session.restore()` 连续调用两次不应报错，也不应重复写出破坏性序列。

## P1：终端异步回调错误恢复

现象：

- `E:\codes\gts\internal\stdlib\terminal.go` 的 `eventLoop()` 在 `onInput` / `onResize` 回调返回 runtime error 时，目前主要行为是打印到 stderr。
- 如果错误发生在 TUI 输入回调里，脚本主循环未必有机会执行自己的 `finally`。
- 这类错误最容易造成 raw mode、鼠标模式、光标隐藏、备用屏残留。

建议：

- `terminal.start()` 支持 `onError(error, session)`，让应用可以决定是否继续。
- 默认策略建议为：回调发生 runtime error 时停止 session 并恢复所有终端模式。
- VM 或解释器退出时调用 `StopTerminalSessionsForVM(vm)` 作为最终兜底，覆盖未被脚本捕获的异常路径。
- 如果未来支持 Ctrl+C / signal hook，也应统一走 session restore。

价值：

- TUI 框架可以更薄，业务代码不必知道各种终端模式的恢复顺序。
- 真实 agent 运行中会有网络错误、渲染错误、工具错误，终端恢复必须比业务错误处理更可靠。
